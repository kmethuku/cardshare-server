...
async function getAll(req, res) {
  try {
    let all = await studyModel.getAll(req.body.UID);
...
async function saveOne(req, res) {
  try {
    let nModified = await studyModel.saveOne(req.body);
...
async function deleteOne(req, res) {
  try {
    let nDeleted = await studyModel.deleteOne(req.body.UID, req.params.id);
